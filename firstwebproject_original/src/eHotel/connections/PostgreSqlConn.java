...
public class  PostgreSqlConn{
		...
		public void getConn(){
			...
			try {
				...
				db = DriverManager.getConnection("jdbc:postgresql://web0.site.uottawa.ca:15432/jbrar057",
						"jbrar057", "So67261496");
		...
		public String[] getuserinforbyempSSN(String param){
			getConn();

			String[] pwd = new String[2];
			
	        try{
	            ps = db.prepareStatement("select * from \"eHotel\".employee where employee_ssn=?");
		...
		public boolean insertNewEmployer(String[] param) {
			getConn();
			
			try {
				st = db.createStatement();
				
				sql = "INSERT INTO \"eHotel\".employee(employee_ssn,employee_name,employee_pwd) VALUES('"+param[0]+"','"+param[1]+"','"+param[2]+"')";
		...
		public boolean insertNewCustomer(String[] param){
	        ...
	        try{
	        	...
	        	sql =  "INSERT INTO \"eHotel\".customer(customer_ssn,customer_name,customer_pwd)  VALUES('"+param[0]+"','"+param[1]+"','"+param[2]+"')";
		...
		public  ArrayList<Room> getAllAvailRooms(){
			...
			try {
				...
				while(rs.next()){
					...
					Room room = new Room(room_no, room_status, rate);
					Rooms.add(room);
				}
			} catch (SQLException e) {
		...
		public String bookRoom(String custName, String roomno, String rate){
			getConn();
			String custSSN="";
